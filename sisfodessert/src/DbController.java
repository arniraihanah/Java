...
public class DbController extends MyConfig {
    ...
    public static List<Produk> getDatabase() {
        ...
        try {
            ...
            query = "SELECT ID,NAMA, STOCK, HARGA FROM tb_product";
    ...
    public static void updateHarga(int id, long harga) {
        connection();
        query = "UPDATE tb_product SET HARGA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean insertData(String nama, long harga, int jumlah) {
        connection();
        query = "INSERT INTO tb_product (NAMA, HARGA, STOCK) VALUES (?, ?, ?)";
        try {
            preparedStatement = connection.prepareStatement(query);
        ...
        connection();
        query = "UPDATE tb_product SET NAMA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
            preparedStatement.setInt(2, id);
